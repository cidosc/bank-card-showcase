...
- `examples/effects-gallery/index.html`：静态对照页（2 张项目提供卡面 + 2 张中性占位卡面 × 4 种效果，附原图基线），既是调参工具也是交付给使用方的**检查入口**；
...
| 卡面（原图平均亮度） | normal | holographic | glitter | silver |
| --- | --- | --- | --- | --- |
| **项目提供 · 低多边形曲奇 + 麦田**（189.9） | Δ +4.3 | Δ +7.2 | Δ +6.2 | Δ −4.6 |
| 中性占位 · 浅绿 panda（212.4） | Δ +1.6 | Δ +3.8 | Δ +4.4 | Δ −7.3（对照独立银色原图 206.5） |
| 中性占位 · 深蓝 dark（44.9） | Δ +4.3 | Δ +6.0 | Δ +4.6 | Δ +9.2 |
| **项目提供 · 低多边形熊猫 + 几何背景**（168.6） | Δ +2.5 | Δ +4.4 | Δ +9.6 | Δ −3.1 |

全部卡面的削波均为 **0%–1.6%**（判据 ≤ 3%），最大亮度偏移 **9.6**（判据 ≤ 12）。
（注：曲奇源图本身带约 7.7% 的高光头削波，属美术自身特性；叠加特效后并未变得更糟。）
